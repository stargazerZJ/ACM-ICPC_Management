...
namespace ICPCManagementSystem {

    void destructICPCManagementSystem();
    ...
    int getProblemID(const std::string &problem_string) {
...
struct ICPCManagementSystem::Team {
    ...
    int *accepted_time_up_;

    Team() : accepted_problems_(0), frozen_problems_(0), penalty_(0), rank_(0), problems_(nullptr), accepted_time_up_(nullptr) {}
    ...
    void initialize(std::string name, int problems, int rank) {
        ...
        accepted_time_up_ = new int[problems];
    ...
    void getAcceptedTime() const {
        ...
        while (mask) {
            ...
            accepted_time_up_[i] = problems_[problem_id].accepted_time_;
            mask ^= 1 << problem_id;
            ++i;
        }
        std::sort(accepted_time_up_, accepted_time_up_ + i);
    }
};

void ICPCManagementSystem::destructICPCManagementSystem() {
    delete[] rankings_pointer_;
    delete[] teams_;
}
...
bool ICPCManagementSystem::compareTeam::operator()(const ICPCManagementSystem::Team *a,
    ...
    for (int i = 0; i < accepted_problem_count; ++i) {
        if (a->accepted_time_up_[i] != b->accepted_time_up_[i]) {
            return a->accepted_time_up_[i] < b->accepted_time_up_[i];
        }
    }
    return a < b;
}
...
bool ICPCManagementSystem::startContest(int duration, int problems) {
    ...
    rankings_pointer_ = new Team *[team_count_];
    int i = 0;
...
#endif
    while (ICPCManagementSystem::CommandHandler());
    ICPCManagementSystem::destructICPCManagementSystem();
    return 0;
}