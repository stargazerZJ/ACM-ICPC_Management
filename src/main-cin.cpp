...
bool ICPCManagementSystem::CommandHandlerCin() {
    static std::string command;
    static std::string team_name;
    static std::string problem_string;
    ...
    if (command == "ADDTEAM") {
        ...
        // START DURATION [duration_time] PROBLEM [problem_count]
        int duration, problems;
        std::cin >> command >> duration >> command >> problems;
        startContest(duration, problems);
    } else if (command == "SUBMIT") {
        // SUBMIT [problem_name] BY [team_name] WITH [submit_status] AT [time]
        std::cin >> problem_string >> command >> team_name >> command >> result_string >> command >> time;
        submitSolution(team_name, problem_string, result_string, time);
        ...
        // QUERY_SUBMISSION [team_name] WHERE PROBLEM=[problem_name] AND STATUS=[status]
        std::cin >> team_name >> command >> problem_string >> command >> result_string;
        querySubmission(team_name, problem_string, result_string);
    } else if (command == "END") {
        // END
        puts("[Info]Competition ends.");
...
int main() {
    ...
    while (ICPC_management_system.CommandHandlerCin());
    return 0;
}