...
private:
    ...
    std::set<Team *, compareTeam> rankings_set_;
...
void ICPCManagementSystem::flush(bool log) {
    ...
    submissions_.clear();
    std::sort(rankings_pointer_, rankings_pointer_ + team_count_, compareTeam());
    for (int rank = 1; rank <= team_count_; ++rank) {
        Team *team = rankings_pointer_[rank - 1];
        team->rank_ = rank;
    }
    if (log)
...
bool ICPCManagementSystem::scroll() {
    ...
    for (int i = 0; i < team_count_; ++i) {
        ...
        rankings_set_.insert(rankings_set_.end(), team);
    ...
    while (!teams_with_frozen_problems.empty()) {
        ...
        rankings_set_.erase(team);
        auto runner_up_before_unfreezing = rankings_set_.upper_bound(team);
        ...
        team->frozen_problems_ ^= 1 << problem_id;
        auto runner_up_after_unfreezing = rankings_set_.upper_bound(team);
        ...
        rankings_set_.insert(runner_up_after_unfreezing, team);
        if (team->frozen_problems_) {
            teams_with_frozen_problems.push(team);
        }
    }
